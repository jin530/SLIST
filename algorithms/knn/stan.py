...
class STAN:
    '''
    STAN( k, sample_size=500, sampling='recent',  remind=False, pop_boost=0, session_key = 'SessionId', item_key= 'ItemId')
    ...
    def __init__(self, k=500, l1=3.54, l2=20, l3=3.54, factor1=True, factor2=True, factor3=True,
                 sample_size=1000, sampling='recent',  remind=False, pop_boost=0, 
        ...
        self.l2 = l2 * 24 * 3600
        self.l3 = l3 
        self.factor1 = factor1 # use or not use l1
        self.factor2 = factor2 # use or not use l2
        self.factor3 = factor3 # use or not use l3
...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
        ...
        # l2
        length = len(self.session_items)
        self.current_session_weight_cache = dict()
        for idx, item in enumerate(self.session_items):
    ...
    def calc_similarity(self, session_items, sessions):
        ...
        for session in sessions:
            ...
            similarity = self.cosine(session_items_test, session_items)
            if self.factor2 is True:
                similarity = similarity * math.exp(
                    -(self.current_timestamp - self.session_time[session]) / self.l2)
            if similarity > 0:
    ...
    def score_items(self, neighbors, session_items):
        '''
